...
namespace MediaWiki\Extension\WikibaseInWikitext\Tests;

use MediaWiki\Extension\WikibaseInWikitext\Hooks;

/**
 * @coversDefaultClass \MediaWiki\Extension\WikibaseInWikitext\Hooks
 */
class HooksTest extends \MediaWikiUnitTestCase {
	...
	public function testOnBeforePageDisplayVandalizeIsTrue() {
		$config = new \HashConfig( [
			'WikibaseInWikitextVandalizeEachPage' => true
		] );
		...
		$outputPageMock->expects( $this->once() )
			->method( 'addHTML' )
			->with( '<p>WikibaseInWikitext was here</p>' );
		$outputPageMock->expects( $this->once() )
			->method( 'addModules' )
			->with( 'oojs-ui-core' );
	...
	public function testOnBeforePageDisplayVandalizeFalse() {
		$config = new \HashConfig( [
			'WikibaseInWikitextVandalizeEachPage' => false
		] );
		$outputPageMock = $this->getMockBuilder( \OutputPage::class )
			->disableOriginalConstructor()
			->getMock();
